...
def _get_keep_latest_ckpts_from_env() -> int:
    raw = str(os.environ.get('KEEP_LATEST_CKPTS', '0') or '0').strip()
    try:
        keep_latest = int(raw)
    except Exception:
        print(f'[CKPTSaver][WARN] invalid KEEP_LATEST_CKPTS={raw}, ignore pruning', flush=True)
        return 0
    return max(0, keep_latest)


def _cleanup_old_global_step_ckpts(ckpt_dir: str, keep_latest: int) -> None:
    if keep_latest <= 0:
        return
    all_ckpt = glob_with_global_step(os.path.join(ckpt_dir, 'global_step_*.pth'))
    if len(all_ckpt) <= keep_latest:
        return
    for old_ckpt in all_ckpt[keep_latest:]:
        try:
            os.remove(old_ckpt)
            print(f'[CKPTSaver] pruned old checkpoint: {old_ckpt}', flush=True)
        except FileNotFoundError:
            continue
        except Exception as exc:
            print(f'[CKPTSaver][WARN] failed to prune {old_ckpt}: {exc}', flush=True)
...
class CKPTSaver(object):
    ...
    def sav(
        ...
        if self.is_master:
            _cleanup_old_global_step_ckpts(args.bed, _get_keep_latest_ckpts_from_env())
