...
def apply_rotary_emb(q, k, rope_cache):
    ...
    # Optional memory guard for long sequences (strict GRPO trace-replay can reach very large token counts).
    # Chunking reduces peak temp allocations inside rotary math (tmp tensors scale with L*head_dim).
    _rope_chunk = int(os.environ.get("INFINITY_GRPO_ROPE_CHUNK", "0") or 0)
    _rope_chunk_minseq = int(os.environ.get("INFINITY_GRPO_ROPE_CHUNK_MINSEQ", "8192") or 0)
    with torch.autocast(device_type=device_type, enabled=False):
        for i in range(2):
            qk[i] = qk[i].reshape(*qk[i].shape[:-1], -1, 2)
            # rope_cache is typically fp32; avoid type promotion (bf16 * fp32 -> fp32) that doubles temp memory.
            rc0, rc1 = rope_cache[0], rope_cache[1]
            if rc0.dtype != qk[i].dtype:
                rc0 = rc0.to(dtype=qk[i].dtype)
                rc1 = rc1.to(dtype=qk[i].dtype)

            L = int(qk[i].shape[-3])
            if _rope_chunk > 0 and qk[i].is_cuda and L >= _rope_chunk_minseq and L > _rope_chunk:
                # Assume rope cache last two dims are (L, dim/2), slice along L when possible.
                for st in range(0, L, _rope_chunk):
                    ed = min(L, st + _rope_chunk)
                    rc0_s = rc0[..., st:ed, :] if (rc0.ndim >= 2 and rc0.shape[-2] >= ed) else rc0
                    rc1_s = rc1[..., st:ed, :] if (rc1.ndim >= 2 and rc1.shape[-2] >= ed) else rc1
                    tmp1 = qk[i][..., st:ed, :, 1] * rc1_s
                    tmp2 = qk[i][..., st:ed, :, 0] * rc1_s
                    qk[i][..., st:ed, :, 0].mul_(rc0_s).sub_(tmp1)
                    qk[i][..., st:ed, :, 1].mul_(rc0_s).add_(tmp2)
            else:
                tmp1 = qk[i][..., 1] * rc1
                tmp2 = qk[i][..., 0] * rc1
                qk[i][..., 0].mul_(rc0).sub_(tmp1)
                qk[i][..., 1].mul_(rc0).add_(tmp2)
