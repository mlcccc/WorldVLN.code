...
class Infinity(nn.Module):
    ...
    def ar_infer_infinity_elegant(
        ...
        forced_idx_trace: Optional[List[Any]] = None,
        # Optional: return per-clip logprob sums (clip0=image, clip1..=video clips).
        return_logprob_by_clip: bool = False,
        ...
        **kwargs,
        ...
        trace_tokens: List[torch.Tensor] = []
        sample_logprob = torch.zeros((B,), dtype=prefix_tokens.dtype, device=prefix_tokens.device)
        sample_logprob_by_clip: Optional[List[torch.Tensor]] = None
        step_clipids: Optional[List[int]] = [] if (return_trace or return_logprob_by_clip) else None
        trace_step_ptr = 0
        ...
        cum_scales = 0
        if return_logprob_by_clip:
            # number of clips is an integer multiple of scales_in_one_clip by construction
            n_clips = max(1, int(len(scale_schedule) // max(1, scales_in_one_clip)))
            sample_logprob_by_clip = [torch.zeros_like(sample_logprob) for _ in range(n_clips)]
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            rel_si_in_one_clip = si % scales_in_one_clip
            clipid = int(si // max(1, scales_in_one_clip))
            ...
            for repeat_idx in range(infer_repeat_times):
                ...
                if forced_idx_trace is not None and trace_step_ptr < len(forced_idx_trace):
                    forced = forced_idx_trace[trace_step_ptr]
                    if not isinstance(forced, torch.Tensor):
                        forced = torch.tensor(forced, dtype=torch.long, device=probs_Bld.device)
                    else:
                        forced = forced.to(device=probs_Bld.device, dtype=torch.long)
                    if forced.ndim == 1:
                        forced = forced.unsqueeze(0)
                    if forced.shape[0] == 1 and tmp_bs > 1:
                        forced = forced.repeat(tmp_bs, 1)
                    idx_Bld = forced.reshape(tmp_bs, -1)
                else:
                ...
                # IMPORTANT: gt_leak should override sampled tokens BEFORE logprob/trace,
                # otherwise trace/logprob won't match the actual tokens used downstream.
                if si < gt_leak and gt_ls_Bl is not None:
                    gt_step = gt_ls_Bl[cum_scales + repeat_idx]
                    if isinstance(gt_step, torch.Tensor):
                        gt_step = gt_step.to(device=probs_Bld.device, dtype=torch.long)
                        idx_Bld = gt_step.reshape(tmp_bs, -1)
                probs_Bld = torch.gather(probs_Bld, dim=2, index=idx_Bld.unsqueeze(-1)).squeeze(-1)
                step_lp = torch.log(probs_Bld.clamp_min(1e-12)).sum(dim=1)
                sample_logprob = sample_logprob + step_lp
                if sample_logprob_by_clip is not None and 0 <= clipid < len(sample_logprob_by_clip):
                    sample_logprob_by_clip[clipid] = sample_logprob_by_clip[clipid] + step_lp
                if return_trace:
                    trace_tokens.append(idx_Bld.detach().to("cpu"))
                if step_clipids is not None:
                    step_clipids.append(int(clipid))
                trace_step_ptr += 1
        ...
        if return_summed_code_only:
            if return_trace:
                out = {
                    ...
                    "idx_trace": trace_tokens,
                    "sample_logprob": sample_logprob.detach().to("cpu"),
                }
                if sample_logprob_by_clip is not None:
                    out["sample_logprob_by_clip"] = [t.detach().to("cpu") for t in sample_logprob_by_clip]
                if step_clipids is not None:
                    out["step_clipids"] = step_clipids
                return out
            return summed_codes
        else:
            ...
            if return_trace:
                return idx_Bl_list, img, {"idx_trace": trace_tokens, "sample_logprob": sample_logprob.detach().to("cpu")}
            return idx_Bl_list, img

    def ar_infer_infinity_elegant_replay_logprob(
        self,
        *,
        vae=None,
        scale_schedule=None,
        ...
        logprob_clipid_target: Optional[int] = None,
        kv_cache_reset: bool = True,
        skip_text_forward: bool = False,
        cache_text_as_gt: bool = False,
        extra_ref_text_scale_inds=None,
    ):
        """
        GRPO trace replay (with grad):
        - Force the autoregressive path to follow `forced_idx_trace` and return
          the summed log-probability of those tokens under the *current* policy.
        - This is designed for PPO/GRPO ratio computation:
            ratio = exp(logp_new - logp_old)
        IMPORTANT:
        - Unlike `ar_infer_infinity_elegant`, this function is NOT wrapped in `torch.no_grad()`.
        - It intentionally does NOT decode images; it only returns `sample_logprob` (Tensor[B]).
        """
        from infinity.schedules.infinity_elegant import interpolate
        # Memory/compute knob (approximate gradient, exact logp):
        # If enabled, we build prefix KV caches under no_grad and only enable grad for the target clip.
        # This can drastically reduce activation memory for long prefixes (e.g. clip3),
        # while keeping the computed logp_new value correct. Gradient becomes a truncated approximation.
        grad_target_clip_only = bool(int(os.environ.get("INFINITY_GRPO_GRAD_TARGET_CLIP_ONLY", "0") or 0) == 1)
        if cfg_list is None:
            cfg_list = []
        if tau_list is None:
            tau_list = []
        if forced_idx_trace is None:
            raise ValueError("replay_logprob requires forced_idx_trace")
        if args is None:
            raise ValueError("replay_logprob requires args")
        if scale_schedule is None:
            raise ValueError("replay_logprob requires scale_schedule")
        if label_B_or_BLT is None:
            raise ValueError("replay_logprob requires label_B_or_BLT")
        if get_visual_rope_embeds is None:
            raise ValueError("replay_logprob requires get_visual_rope_embeds")
        ...
        assert len(cfg_list) >= len(scale_schedule)
        assert len(tau_list) >= len(scale_schedule)
        assert int(getattr(args, "use_cfg", 1)) + int(getattr(args, "use_apg", 0)) == 1
        ...
        # Rope cache for text step
        self.rope2d_freqs_grid["freqs_text"] = self.rope2d_freqs_grid["freqs_text"].to(device)
        text_maxlen_this_iter = label_B_or_BLT[-1]
        prefix_tokens, lens = self.prepare_text_conditions(
            label_B_or_BLT,
            cfg_list,
            B,
            negative_label_B_or_BLT,
        ...
        sample_logprob = torch.zeros((B,), dtype=prefix_tokens.dtype, device=prefix_tokens.device)
        sample_logprob_by_clip: Dict[int, torch.Tensor] = {}
        trace_step_ptr = 0

        for b in self.unregistered_blocks:
            b.attn.kv_caching(True, reset=kv_cache_reset)
        first_full_spatial_size_scale_index = get_first_full_spatial_size_scale_index(scale_schedule)
        image_scale_repetition = np.array(json.loads(args.image_scale_repetition))
        video_scale_repetition = np.array(json.loads(args.video_scale_repetition))
        scales_in_one_clip = first_full_spatial_size_scale_index + 1
        ...
        assert len(image_scale_repetition) == scales_in_one_clip, f"{len(image_scale_repetition)} != {scales_in_one_clip}"
        block_chunks = self.block_chunks if self.num_block_chunks > 1 else self.blocks
        # Engineering approximation (memory): only train the last N transformer blocks during replay.
        # Forward remains exact (logp_new value unchanged), but gradients through earlier blocks are truncated.
        try:
            train_last_n_blocks = int(os.environ.get("INFINITY_GRPO_TRAIN_LAST_N_BLOCKS", "0") or 0)
        except Exception:
            train_last_n_blocks = 0
        _cutoff_block = max(0, int(len(block_chunks)) - int(train_last_n_blocks)) if train_last_n_blocks > 0 else 0

        # Early-stop for strict GRPO:
        # When only a single clip's new_logprob is requested, tokens from *later* clips do not affect
        # the logprob of earlier clips (causal). We can stop replay after finishing the target clip to
        # reduce KV-cache growth, peak memory, and runtime.
        end_si_inclusive = None
        if logprob_clipid_target is not None:
            try:
                tgt = int(logprob_clipid_target)
                if tgt >= 0:
                    end_si_inclusive = min(len(scale_schedule) - 1, (tgt + 1) * scales_in_one_clip - 1)
            except Exception:
                end_si_inclusive = None
        start_si_grad = None
        if grad_target_clip_only and logprob_clipid_target is not None:
            try:
                tgt = int(logprob_clipid_target)
                if tgt >= 0:
                    # By default, enable grad for the whole target clip.
                    start_of_clip = min(len(scale_schedule) - 1, tgt * scales_in_one_clip)
                    start_si_grad = start_of_clip
                    # Optional: only keep grad for the last N scales within the target clip (more aggressive memory saving).
                    tail_scales = int(os.environ.get("INFINITY_GRPO_GRAD_TARGET_TAIL_SCALES", "0") or 0)
                    if tail_scales > 0 and end_si_inclusive is not None:
                        start_si_grad = max(start_of_clip, int(end_si_inclusive) - tail_scales + 1)
            except Exception:
                start_si_grad = None
        ...
        # NOTE (memory): replay_logprob does not need the full latent history (no decoding / no return of summed_codes).
        # Keeping a growing `summed_codes` list (as in inference) retains multiple latent chunks on GPU and can
        # contribute to OOM in strict GRPO trace-replay. We only keep the current latent chunk.
        summed_code = noise[0:1]
        sos_token = self.embeds_codes2input(noise, bs // 1)

        # Replay memory control: activation checkpointing.
        # IMPORTANT: this replay path uses stateful KV caching (writes/clears caches during forward),
        # which is not compatible with torch.utils.checkpoint and can raise CheckpointError.
        # Therefore we keep it disabled by default and only enable when explicitly requested AND
        # you have verified it works for your setup.
        use_replay_ckpt = bool(int(getattr(args, "grpo_replay_checkpointing", 0) or 0) == 1) and (
            str(getattr(args, "enable_checkpointing", "") or "") == "full-block"
        )
        if use_replay_ckpt:
            import torch.utils.checkpoint as _ckpt

        # Optional text forward (for caching prefix as GT)
        if not skip_text_forward:
            # If we only train on the target clip, do not build an autograd graph for the long text/prefix forward.
            text_ctx = torch.no_grad() if (grad_target_clip_only and logprob_clipid_target is not None) else nullcontext()
            if cache_text_as_gt:
                self.set_cache_write_is_pred(False)
            rope_cache = self.rope2d_freqs_grid["freqs_text"][:, :, :, :, :text_maxlen_this_iter]
            with text_ctx:
                last_stage = prefix_tokens
                for bi, b in enumerate(block_chunks):
                    # If enabled, stop-grad for early blocks even when text_ctx allows grad.
                    blk_grad_on = True
                    if train_last_n_blocks > 0 and bi < _cutoff_block:
                        blk_grad_on = False
                    if use_replay_ckpt:
                        def _fn(x):
                            return b(
                        ...
                        with torch.set_grad_enabled(blk_grad_on and torch.is_grad_enabled()):
                            last_stage = _ckpt.checkpoint(_fn, last_stage, use_reentrant=False)
                    else:
                        with torch.set_grad_enabled(blk_grad_on and torch.is_grad_enabled()):
                            last_stage = b(
                                x=last_stage,
        ...
        try:
            for si, pn in enumerate(scale_schedule):
                ...
                # Optional truncated-gradient mode: only keep autograd for target clip steps.
                grad_on = True
                if grad_target_clip_only and start_si_grad is not None:
                    grad_on = bool(si >= int(start_si_grad))

                with torch.set_grad_enabled(grad_on):
                    cfg = cfg_list[si]
                    infer_repeat_times = min(repeat_times, int(getattr(args, "max_repeat_times", repeat_times)))
                    for repeat_idx in range(infer_repeat_times):
                        ...
                        logits_BlV = self.get_logits_during_infer(
                            last_stage, is_semantic_scale=rel_si_in_one_clip < args.semantic_scales
                        ).mul(1 / tau_list[si])
                        if cfg != 1:
                            if int(getattr(args, "use_cfg", 1)):
                                logits_BlV = cfg * logits_BlV[:B] + (1 - cfg) * logits_BlV[B:]
                            else:
                                pred_cond = logits_BlV[:B]
                                pred_uncond = logits_BlV[B:]
                                pred_guided = normalized_guidance(
                                    ...
                                    eta=0,
                                    norm_threshold=getattr(args, "apg_norm_threshold", 0.0),
                                )
                                logits_BlV = pred_guided
                        else:
                            logits_BlV = logits_BlV[:B]
                        ...
                        tmp_bs, tmp_seq_len = logits_BlV.shape[:2]
                        logits_BlV = logits_BlV.reshape(tmp_bs, -1, self.num_of_label_value)
                        probs_full_Bld = logits_BlV.softmax(dim=-1)  # [B, L, 2]

                        # Force replay token
                        if trace_step_ptr >= len(forced_idx_trace):
                            raise RuntimeError(
                                f"forced_idx_trace exhausted at step={trace_step_ptr} (need >= {trace_step_ptr+1})"
                            )
                        forced = forced_idx_trace[trace_step_ptr]
                        if not isinstance(forced, torch.Tensor):
                            forced = torch.tensor(forced, dtype=torch.long, device=probs_full_Bld.device)
                        else:
                            forced = forced.to(device=probs_full_Bld.device, dtype=torch.long)
                        if forced.ndim == 1:
                            forced = forced.unsqueeze(0)
                        if forced.shape[0] == 1 and tmp_bs > 1:
                            forced = forced.repeat(tmp_bs, 1)
                        ...
                        # logprob on forced token
                        probs_taken_Bld = torch.gather(probs_full_Bld, dim=2, index=idx_Bld.unsqueeze(-1)).squeeze(-1)
                        step_lp = torch.log(probs_taken_Bld.clamp_min(1e-12)).sum(dim=1)
                        sample_logprob = sample_logprob + step_lp
                        sample_logprob_by_clip[clipid] = sample_logprob_by_clip.get(
                            clipid, torch.zeros_like(sample_logprob)
                        ) + step_lp
                        trace_step_ptr += 1

                        def Bld2Bthwd(item):
                            item = item.reshape(tmp_bs, tmp_seq_len, -1)
                            item = item.reshape(B, pn[0], pn[1], pn[2], -1)
                            if self.apply_spatial_patchify:
                                item = item.permute(0, 1, 4, 2, 3)
                                item = torch.nn.functional.pixel_shuffle(item, 2)
                                item = item.permute(0, 1, 3, 4, 2)
                            return item

                        idx_Bthwd = Bld2Bthwd(idx_Bld)

                        # Apply official leak (same semantics as inference) if provided.
                        if si < gt_leak and gt_ls_Bl is not None:
                            idx_Bthwd = gt_ls_Bl[cum_scales + repeat_idx].to(
                                device=idx_Bthwd.device, dtype=idx_Bthwd.dtype
                            )
                        ...
                        summed_code = F.interpolate(summed_code, size=target_pn, mode=vae.quantizer.z_interplote_up)
                        summed_code = summed_code + codes
                        if repeat_idx < repeat_times - 1:
                            last_stage = F.interpolate(
                                summed_code, size=vae_scale_schedule[si], mode=vae.quantizer.z_interplote_down
                            )
                            last_stage = self.embeds_codes2input(last_stage, bs // B)

                cum_scales += repeat_times

                # Early-stop once target clip is finished (saves memory/time).
                if end_si_inclusive is not None and si >= int(end_si_inclusive):
                    break

                if si < len(scale_schedule) - 1:
                    if scale_schedule[si][-2:] == scale_schedule[-1][-2:]:
                        if self.other_args.noise_input:
                            summed_code = torch.randn(
                                (B, summed_code.shape[1], *vae_scale_schedule[si + 1]),
                                device=summed_code.device,
                                dtype=summed_code.dtype,
                            )
                        else:
                            summed_code = torch.zeros(
                                (B, summed_code.shape[1], *vae_scale_schedule[si + 1]),
                                device=summed_code.device,
                                dtype=summed_code.dtype,
                            )
                        last_stage = summed_code
                    else:
                        last_stage = F.interpolate(
                            summed_code, size=vae_scale_schedule[si + 1], mode=vae.quantizer.z_interplote_down
                        )
                    last_stage = self.embeds_codes2input(last_stage, bs // B)
        finally:
            for b in self.unregistered_blocks:
                b.attn.kv_caching(False, reset=kv_cache_reset)
        if logprob_clipid_target is None:
            return sample_logprob
        return sample_logprob_by_clip.get(int(logprob_clipid_target), torch.zeros_like(sample_logprob))
