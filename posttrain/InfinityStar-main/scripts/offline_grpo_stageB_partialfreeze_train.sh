...
export FAST_OUT_DIR="${FAST_OUT_DIR:-${OUTPUT_ROOT}/offline_grpo_data_fast}"
...
if [ "${SP_SIZE_ENV_SET}" -eq 0 ]; then
  export SP_SIZE="$(offline_grpo_recommend_stageb_sp_size "${OFFLINE_GRPO_TOTAL_GPUS}")"
else
  export SP_SIZE="${SP_SIZE}"
fi
if [ "${AC_ENV_SET}" -eq 0 ]; then
  export AC="$(offline_grpo_recommend_stageb_ac "${default_ac}" "${OFFLINE_GRPO_TOTAL_GPUS}" "${SP_SIZE}")"
else
  export AC="${AC}"
fi
if [ "${GRPO_NEW_LOGPROB_MODE}" != "trace_replay" ] && [ "$((OFFLINE_GRPO_TOTAL_GPUS % SP_SIZE))" -ne 0 ]; then
  echo "[FATAL] total_gpus=${OFFLINE_GRPO_TOTAL_GPUS} must be divisible by SP_SIZE=${SP_SIZE}"
  exit 1
fi
echo "[stageB_partialfreeze] mode=${PARTIAL_FREEZE_MODE} total_gpus=${OFFLINE_GRPO_TOTAL_GPUS} nnodes=${NNODES} nproc_per_node=${NPROC_PER_NODE} sp_size=${SP_SIZE} ac=${AC} video_batch_size=${VIDEO_BATCH_SIZE}"

bash "${ROOT_DIR}/scripts/offline_grpo_stageB_train.sh"