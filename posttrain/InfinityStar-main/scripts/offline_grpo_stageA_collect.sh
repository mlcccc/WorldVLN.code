...
# Usage:
#   bash scripts/offline_grpo_stageA_collect.sh
#
...
. "${ROOT_DIR}/scripts/offline_grpo_cluster_env.sh"
...
FAST_OUT_DIR="${FAST_OUT_DIR:-${OUTPUT_ROOT}/offline_grpo_data_fast}"
...
ROLLOUT_TASK_BUILDER_PY="${ROLLOUT_TASK_BUILDER_PY:-${ROOT_DIR}/tools/offline_grpo/build_rollout_tasks.py}"
ROLLOUT_TRAJECTORY_PY="${ROLLOUT_TRAJECTORY_PY:-${ROOT_DIR}/tools/offline_grpo/generate_candidate_trajectories_real.py}"
ROLLOUT_REWARD_PY="${ROLLOUT_REWARD_PY:-${ROOT_DIR}/tools/offline_grpo/reward_uavflow.py}"
...
UAVFLOW_STAGEA_ROLLOUT_BACKEND="${UAVFLOW_STAGEA_ROLLOUT_BACKEND:-offline_video}"
...
if [ -z "${STAGEA_GPU_IDS}" ]; then
  STAGEA_GPU_IDS="${OFFLINE_GRPO_LOCAL_GPU_IDS}"
fi
...
STAGEA_FAIL_MARKER="${STAGEA_SYNC_ROOT}/${OFFLINE_GRPO_NODE_TAG}.fail"
...
if [ "${STAGEA_LOCAL_LOG_FILE_ENV_SET}" -eq 0 ]; then
  STAGEA_LOCAL_LOG_FILE="${STAGEA_LOCAL_LOG_ROOT}/${OFFLINE_GRPO_NODE_TAG}/stageA_${RUN_ID}.log"
fi
...
PY
  ...
  "${PYTHON_BIN}" "${ROOT_DIR}/tools/offline_grpo/generate_candidate_rollouts.py" \
...
if [ "${NNODES}" -gt 1 ]; then
  if [ "${NODE_RANK}" -eq 0 ]; then
    ...
    offline_grpo_wait_for_path "${STAGEA_READY_FILE}" "${STAGEA_SYNC_TIMEOUT}" "stageA rollout inputs" "${STAGEA_FAIL_GLOB}"
  fi
fi
...
if [ "${USE_REAL_ROLLOUT}" = "1" ]; then
  ...
  if [ "${NODE_RANK}" -eq 0 ]; then
    ...
    "${PYTHON_BIN}" "${ROOT_DIR}/tools/offline_grpo/generate_candidate_trajectories.py" \
      --candidates_jsonl "${CAND_JSONL}" \
      --trajectory_root "${TRAJ_DIR}" \
      --pos_noise_std 0.02 \
...
if [ "${NNODES}" -gt 1 ] && [ "${NODE_RANK}" -eq 0 ]; then
  offline_grpo_wait_for_markers "${STAGEA_SYNC_ROOT}" "rollout_node_" "${NNODES}" "${STAGEA_SYNC_TIMEOUT}" "${STAGEA_FAIL_GLOB}"
fi
...
if [ "${NODE_RANK}" -eq 0 ]; then
  ...
  "${PYTHON_BIN}" "${ROOT_DIR}/tools/offline_grpo/build_replay_dataset.py" \
...
PY
  ...
  "${PYTHON_BIN}" "${ROOT_DIR}/tools/offline_grpo/summarize_replay_meta.py" \
...
PY
  ...
  offline_grpo_wait_for_path "${STAGEA_COMPLETE_FILE}" "${STAGEA_SYNC_TIMEOUT}" "stageA completion marker" "${STAGEA_FAIL_GLOB}"
fi
