...
# Minimal offline-GRPO training launcher (single or multi node).
...
# Trainer mode:
# - offline_grpo: strict GRPO/PPO-style objective on offline rollouts
# - sft: standard teacher-forcing training (ignores GRPO fields)
TRAINER_TYPE="${TRAINER_TYPE:-offline_grpo}"
...
TRAIN_EXP_NAME="${TRAIN_EXP_NAME:-offline_grpo_uavflow_mvp}"
...
echo "[stageB] run_id=${RUN_ID:-latest} nnodes=${NNODES} node_rank=${NODE_RANK} nproc_per_node=${NPROC_PER_NODE} total_gpus=${OFFLINE_GRPO_TOTAL_GPUS} sp_size=${SP_SIZE} ac=${AC} replay_meta_dir=${REPLAY_META_DIR}"
