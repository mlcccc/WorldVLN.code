...
def _read_frames_rgb(video_path: str, num_frames: int = 49) -> List[Any]:
...
def _frame_rgb_to_b64(frame_rgb) -> str:
...
def main():
    ...
    ap.add_argument(
        "--rollout_backend",
        type=str,
        default=str(os.environ.get("UAVFLOW_STAGEA_ROLLOUT_BACKEND", "offline_video")),
        choices=["offline_video", "remote_sim"],
        help="offline_video=read saved reference video; remote_sim=locally infer and step a remote UAV-Flow simulator service.",
    )
    ap.add_argument(
        "--simulator_base_url",
        type=str,
    ...
    with open(os.path.abspath(args.candidates_jsonl), "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                if rollout_backend == "offline_video":
                    frames_rgb = _read_frames_rgb(video_path, num_frames=int(args.num_frames))
                    b64 = [_frame_rgb_to_b64(fr) for fr in frames_rgb]
                else:
                    b64 = []
                ...
                for attempt in range(max_retry):
                    ...
                    try:
                        if rollout_backend == "remote_sim":
                            ...
                            r0 = mod._predict_delta_actions_impl(req0)
                            dt0 = time.perf_counter() - t0

                            # seg01 from prefix 1..17
                            ...
                            r1 = mod._predict_delta_actions_impl(req1)
                            dt1 = time.perf_counter() - t1

                            # seg02 from prefix 1..33 (future-last enabled)
                            ...
                            r2 = mod._predict_delta_actions_impl(req2)
